...
type runCmdOutputMatcher struct {
	outputMatcher  types.GomegaMatcher
...
func RunSuccessfullyWithOutputString(outputMatcher types.GomegaMatcher) types.GomegaMatcher {
...
func (m *runCmdOutputMatcher) Match(actual interface{}) (bool, error) {
	...
	output := string(m.runCmdMatcher.session.Buffer().Contents())

	if ok, err := m.outputMatcher.Match(output); !ok {
		m.failureMessage = m.outputMatcher.FailureMessage(output)
		return false, err
	}
